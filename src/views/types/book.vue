...
<template>
  ...
  <Card :arrIndex="arr.titles" @titleClick="changeData" />
  <Book :arrData="arr.data[tempIndex]" />
</template>

<script setup>
import { ref, reactive, computed } from "vue";
import Book from "@/components/article.vue";
...
const doubleValue = computed(() => mystore.double)
const { count } = storeToRefs(mystore)
// // const count = computed(() => mystore.count);

// const { count } = toRefs(mystore)
function addNum() {
  mystore.increment()
}
...
const arr = reactive({
  titles: [
    { title: '滕王阁序', author: '(唐-王勃)' },
    { title: '出师表', author: '(三国-诸葛亮)' },
    { title: '蜀道难', author: '(唐-李白)' },
    { title: '蜀道难', author: '(唐-李白)' },
    { title: '蜀道难', author: '(唐-李白)' },
    { title: '蜀道难', author: '(唐-李白)' },
  ],
  data: [
    ...
    {
      title: '出师表',
      article: 'sadfhasfdhajdfahjdfkadfhakjfdkj'
    },
    {
      title: '蜀道难',
      article: 'sadfhasfdhajdfahjdfkadfhakjfdkj'
